...
- The same RF-read preissue path is used while CVFPU operations wait for their
  result.
...
- The RF read ports prearm to the queued decode head, allowing a retire-stage
  queued decode launch to advance directly to RF3 when the address is already
  sampled by the synchronous register file.
...
- Integer load completions use the helper; FP loads intentionally remain on the
  older fetch path because their NaN-boxing/writeback path has extra state.
- CBO completion/non-cacheable no-op exits use the same no-writeback helper.
