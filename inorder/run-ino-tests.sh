...
#   ./run-ino-tests.sh [class-glob ...]      e.g. rv64ui-p rv64um-p
...
# Shared modules compile straight out of ../probe -- inorder/ never edits them.
#
...
# through ./run-ino-vl.sh (verilator + the real CVFPU) instead.
...
iverilog -g2012 -I. -I../probe -I../src -s tb -o /tmp/ino_riscv.vvp \
   ino_core.v ino_frontend.v ino_exec.v ino_lsu.v ino_regfile.v \
   $PROBE_SRCS ../src/alu.v tb_ino_riscv.v || exit 1
...
for cls in "${classes[@]}"; do
   for bin in "$TESTDIR"/${cls}*.bin; do
      ...
      out=$(vvp /tmp/ino_riscv.vvp +hex="$tmp/img.hex" +tohost="$th" +cycles=$CYC 2>&1 \
            | grep -E 'RISCV-TEST')
      case "$out" in
