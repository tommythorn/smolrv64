...
// OOC timing probe for the bypassed 2-stage execute. flopwrap measures the worst
// internal reg-to-reg path, i.e. the worse of RR (RF read) and EX (bypass+ALU).
// IN_W=1029, OUT_W=139.
module probe_dut (input wire clk, input wire [1028:0] din, output wire [138:0] dout);
   ...
   wire [7:0]   byp_valid   = din[460:453];
   wire [55:0]  byp_pr      = din[516:461];
   wire [511:0] byp_val     = din[1028:517];

   wire        wb_valid; wire [6:0] wb_pr; wire [63:0] wb_val;
   wire [63:0] agu_addr; wire cmp_eq, cmp_lt, cmp_ltu;

   exec_shard_bp #(.NB(8)) dut
     (.clk(clk), .iss_valid(iss_valid), .iss_pdst(iss_pdst), .iss_pdst_v(iss_pdst_v),
      ...
      .byp_valid(byp_valid), .byp_pr(byp_pr), .byp_val(byp_val),
...
module exec_shard_bp_probe (input wire clk, output wire probe_out);
   flopwrap #(.IN_W(1029), .OUT_W(139)) u (.clk(clk), .probe_out(probe_out));
endmodule
