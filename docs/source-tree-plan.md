...
1. **The in-order core** — `inorder/`, what currently ships to the XCKU5P and boots Linux.
...
- **Parameterising instead of forking.** `predictor.v` grew a `CKPT_RAS` knob so the
  ...
  was `inorder/ino_predictor.v` — a fork with the whole mechanism deleted. Done in the
...
between the in-order core and the sharded OoO core, FORK IT into `inorder/` rather than
...
The cost of a fork is drift (`inorder/ino_cache.v` is a fork of `src/cache.v` and has
...
Not the predictor: it is a hint structure, and `ino_predictor.v` keeps two committed scalars
