...
module exec_bundle
  #(parameter SHARDS = 4,
   ...
   (input  wire                    clk,
    ...
    input  wire [22:0]             hpm_ev,           // Zihpm event pulses -> csr_file. Width tracks
   ...
   csr_file u_csr
     (.clk(clk), .reset(reset), .dbg_timer(dbg_timer), .dbg_mtvec(dbg_mtvec), .dbg_mtvec_we(dbg_mtvec_we), .dbg_csrop(dbg_csrop), .dbg_csrop_v(dbg_csrop_v),            // squash must NOT reset CSR state (only reset does)
      ...
      .hw_ip(hw_ip), .mtime(mtime), .retire_cnt(retire_cnt), .hpm_retire_cnt(retire_cnt),
      ...
      .hpm_ev(hpm_ev),   // width tracks csr_file's port; backend_top zero-extends at the source
