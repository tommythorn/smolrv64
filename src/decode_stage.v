...
module decode_stage #(parameter IW = 4, parameter SEQW = 8,
   ...
   (input  wire [IW*32-1:0]      inst,
    ...
    output wire [IW*ABITS-1:0]   rs3,
    output wire [IW-1:0]         rs3_v,
    ...
    output wire [IW-1:0]         is_store,
    output wire [IW*2-1:0]       mem_size,
      ...
      for (g = 0; g < IW; g = g + 1) begin : lane
         decode_slot #(.SEQW(SEQW)) ds
           (.inst(inst[g*32 +: 32]), .in_valid(in_valid[g]),
            ...
            .rs3(rs3[g*ABITS +: ABITS]), .rs3_v(rs3_v[g]),
   ...
   decode_xslot #(.IW(IW), .ABITS(ABITS), .SBITS(SBITS)) xs
     (.rs1(rs1), .rs1_v(rs1_v), .rs2(rs2), .rs2_v(rs2_v), .rs3(rs3), .rs3_v(rs3_v),
      .rd(rd), .rd_v(rd_v),
      .s1_is_slot(s1_is_slot), .s1_slot(s1_slot),
      .s2_is_slot(s2_is_slot), .s2_slot(s2_slot),
      .s3_is_slot(s3_is_slot), .s3_slot(s3_slot), .map_writer(map_writer),
