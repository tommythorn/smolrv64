...
// the default geometry IW=4, ABITS=6, SBITS=2. IN_W=84, OUT_W=40.
module probe_dut (input  wire        clk,
   ...
   assign dout = {d_slot, d_is_slot, map_writer, s2_slot, s2_is_slot, s1_slot, s1_is_slot};
endmodule

module decode_xslot_probe (input wire clk, output wire probe_out);
   flopwrap #(.IN_W(84), .OUT_W(40)) u (.clk(clk), .probe_out(probe_out));
endmodule
