...
#
#   ./run-ooo2-linux.sh              boot with the defaults
#   CYC=0 ./run-ooo2-linux.sh        unbounded (wrap in `timeout`!)
#   BUILD=1 ./run-ooo2-linux.sh      force a rebuild
...
#     INITRD=/tmp/tiny128-raw.cpio ./run-ooo2-linux.sh
...
MEM_LG2=${MEM_LG2:-29}
CYC=${CYC:-200000000}
BIN=$(pwd)/obj_dir_ooo2_linux/tb_ooo2_linux
