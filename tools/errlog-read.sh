...
#
# The bit names are the INTEGRITY LOG tables in rv_cache.v and ooo2_lsu.v, in the SoC's
# fixed assignment (D$ [15:0], I$ [31:16], LSU [47:32]); keep the three in step. On a
...
name() {   # <bit> -> unit.name and description
   ...
   elif [ "$b" -lt 32 ]; then echo "icache.${CACHE[$((b-16))]:-?}|${CDESC[$((b-16))]:-}"
